...
def update_firewall_rule(name: str, rule_number: int):
    ...
    _log_commands(f"delete existing rule {rule_number} in {name}", delete_paths)
    success, error_message = configure_delete(delete_paths, error_context=f"replace firewall rule {rule_number} in {name}")
    if not success:
        return _error(error_message or "Failed to update firewall rule.", 500)

    commands = build_rule_set_commands(name, target_number, payload)
    _log_commands(f"create updated rule {target_number} in {name}", commands)
...
def reorder_firewall_rules(name: str):
    ...
    _log_commands(f"delete rules for reorder in {name}", delete_paths)
    success, error_message = configure_delete(delete_paths, error_context=f"reorder firewall rules in {name}")
    if not success:
        return _error(error_message or "Failed to remove firewall rules prior to reorder.", 500)

    _log_commands(f"create reordered rules in {name}", set_commands)
    success, error_message = configure_set(set_commands, error_context=f"reorder firewall rules in {name}")
    if not success:
        restore_commands: List[List[str]] = []
        for rule_id, rule_cfg in snapshots.items():
            restore_commands.extend(flatten_rule_config(ensure_mapping(rule_cfg), name, rule_id))
        if restore_commands:
