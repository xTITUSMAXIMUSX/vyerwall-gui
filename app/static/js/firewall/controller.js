(function initFirewallController(root) {
  ...
  function bindToggleButton() {
    const toggle = document.getElementById('editRuleDisabledToggle');
    if (!toggle) return;

    toggle.addEventListener('click', () => {
      const isCurrentlyEnabled = toggle.dataset.enabled === 'true';
      const willBeEnabled = !isCurrentlyEnabled;

      toggle.dataset.enabled = willBeEnabled ? 'true' : 'false';
      const hiddenInput = document.getElementById('editRuleDisabled');
      const statusLabel = document.getElementById('editRuleStatusLabel');

      if (hiddenInput) {
        hiddenInput.value = willBeEnabled ? 'false' : 'true';
      }
      ...
      if (willBeEnabled) {
        ...
        toggle.querySelector('span').classList.add('translate-x-8');
        if (statusLabel) {
        ...
        toggle.querySelector('span').classList.remove('translate-x-8');
